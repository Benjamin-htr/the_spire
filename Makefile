...
PROJECT_NAME          ?= the_spire
PROJECT_VERSION       ?= 1.0
PROJECT_BUILD_PATH    ?= ./build
PROJECT_SOURCE_FILES  ?= $(shell find src -name "*.c")

# raylib library variables
RAYLIB_SRC_PATH       ?= /Users/benjamin_htr/IT/raylib/src
...
ifeq ($(PLATFORM_OS),WINDOWS)
    ifeq ($(PLATFORM),PLATFORM_WEB)
        ...
        EMSDK_PATH         ?= /Users/benjamin_htr/emsdk
...
ifeq ($(PLATFORM),PLATFORM_WEB)
    ...
    # Export required symbols for runtime JS helpers (e.g., file preloading) that expect Module._malloc/_free
    # NOTE: Emscripten expects a JSON-like list string for EXPORTED_FUNCTIONS
    LDFLAGS += -sUSE_GLFW=3 -sINITIAL_MEMORY=$(BUILD_WEB_HEAP_SIZE) -sSTACK_SIZE=$(BUILD_WEB_STACK_SIZE) -sFORCE_FILESYSTEM=1 -sMINIFY_HTML=0 \
               -sEXPORTED_FUNCTIONS="['_main','_malloc','_free']" \
               -sEXPORTED_RUNTIME_METHODS="['ccall','cwrap','HEAPF32','HEAPU8','HEAP8','FS','requestFullscreen']"
    ...
    ifeq ($(BUILD_MODE),DEBUG)
        LDFLAGS += -sASSERTIONS=2 -sSAFE_HEAP=1 -sSTACK_OVERFLOW_CHECK=2 --profiling
        # Bump memory sizes in DEBUG to rule out low stack/heap issues
        LDFLAGS := $(filter-out -sINITIAL_MEMORY=% -sSTACK_SIZE=%,$(LDFLAGS))
        LDFLAGS += -sINITIAL_MEMORY=192MB -sSTACK_SIZE=2MB
    endif
